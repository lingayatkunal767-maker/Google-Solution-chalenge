...
export default function LandingPage() {
  ...
  return (
    <div className='bg-background text-on-background font-body-md text-body-md antialiased min-h-screen flex flex-col' style={{ background: 'var(--bg-primary)' }}>
      <header className='bg-slate-950/80 backdrop-blur-md border-b border-slate-800/50 shadow-xl shadow-blue-900/5 fixed top-0 w-full z-50 font-manrope antialiased tracking-tight'>
        <div className='max-w-7xl mx-auto flex justify-between items-center px-6 h-20 text-blue-500 dark:text-blue-400'>
          ...
          <nav className='hidden md:flex items-center gap-8'>
            <a className='text-slate-400 font-medium hover:text-slate-200 hover:bg-slate-800/40 transition-all duration-300 px-3 py-2 rounded-md active:scale-95 duration-150 ease-in-out' href='#'>Features</a>
            <a className='text-slate-400 font-medium hover:text-slate-200 hover:bg-slate-800/40 transition-all duration-300 px-3 py-2 rounded-md active:scale-95 duration-150 ease-in-out' href='#'>Solutions</a>
            <a className='text-slate-400 font-medium hover:text-slate-200 hover:bg-slate-800/40 transition-all duration-300 px-3 py-2 rounded-md active:scale-95 duration-150 ease-in-out' href='#'>Pricing</a>
            <a className='text-slate-400 font-medium hover:text-slate-200 hover:bg-slate-800/40 transition-all duration-300 px-3 py-2 rounded-md active:scale-95 duration-150 ease-in-out' href='#'>Resources</a>
          </nav>
          
          {/* Actions */}
          <div className='flex items-center gap-4'>
            <button onClick={() => navigate('/login')} className='hidden md:block text-slate-400 font-medium hover:text-slate-200 hover:bg-slate-800/40 transition-all duration-300 px-4 py-2 rounded-md active:scale-95 duration-150 ease-in-out'>
              Sign In
            </button>
            <button onClick={() => navigate('/login')} className='bg-primary-container text-on-primary-container hover:bg-primary-container/90 transition-all duration-300 px-5 py-2.5 rounded-lg font-title-sm text-title-sm active:scale-95 duration-150 ease-in-out'>
              Get Started
            </button>
          </div>
        </div>
      ...
      <main className='flex-grow pt-24 pb-xl px-margin md:px-xl max-w-7xl mx-auto w-full flex flex-col gap-xl'>
        ...
        <section className='relative w-full rounded-xl bg-surface-container-lowest border border-surface-variant overflow-hidden flex flex-col items-center text-center justify-center p-lg md:p-xl min-h-[500px] md:min-h-[600px] gap-lg'>
          ...
          <div className='relative z-10 flex flex-col items-center gap-md max-w-3xl w-full'>
            ...
            <h1 className='font-display-lg text-display-lg text-on-surface md:text-[56px] md:leading-[64px] tracking-tight'>
              Connect with Causes You Care About
            </h1>
            
            <p className='font-body-md text-body-md text-on-surface-variant max-w-xl text-lg'>
              Easily find volunteer opportunities, coordinate events, and track your impact. The simple way to connect volunteers with organizations.
            </p>
            
            <div className='flex flex-col sm:flex-row justify-center gap-sm mt-sm'>
              <button onClick={() => navigate('/login')} className='bg-primary text-on-primary font-title-sm text-title-sm px-lg py-3 rounded-lg hover:bg-primary-fixed transition-colors flex items-center justify-center gap-sm shadow-lg shadow-primary/20'>
                Get Started
                <span className='material-symbols-outlined text-[20px]'>arrow_forward</span>
              </button>
              <button onClick={() => navigate('/login')} className='bg-surface-container text-on-surface font-title-sm text-title-sm px-lg py-3 rounded-lg border border-outline-variant hover:bg-surface-container-high transition-colors flex items-center justify-center gap-sm'>
                Learn More
              </button>
            </div>
          </div>
        ...
        {/* Trusted By */}
        <section className='flex flex-col items-center gap-md py-lg border-y border-surface-variant/50'>
          <p className='font-label-caps text-label-caps text-on-surface-variant uppercase tracking-widest'>Trusted by Global Organizations</p>
          <div className='flex flex-wrap justify-center items-center gap-xl md:gap-24 opacity-60 grayscale hover:grayscale-0 transition-all duration-500'>
            <span className='font-headline-md text-headline-md text-on-surface'>Acme Corp</span>
            <span className='font-headline-md text-headline-md text-on-surface'>GlobalTech</span>
            <span className='font-headline-md text-headline-md text-on-surface'>Stark Ind.</span>
            <span className='font-headline-md text-headline-md text-on-surface'>Wayne Ent.</span>
          </div>
        </section>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-md md:gap-lg auto-rows-[250px]'>
            ...
            <div className='col-span-1 md:col-span-1 md:row-span-2 bg-surface-container rounded-xl border border-surface-variant p-lg flex flex-col gap-md relative overflow-hidden group hover:border-outline transition-colors'>
              ...
              <div className='flex flex-col gap-xs relative z-10 mt-auto'>
                ...
                <p className='font-body-sm text-body-sm text-on-surface-variant'>
                  Easily align volunteer skills, availability, and location with mission requirements.
                </p>
              </div>
            </div>

            {/* Feature 2: Tracking (Wide) */}
            <div className='col-span-1 md:col-span-2 bg-surface-container rounded-xl border border-surface-variant p-lg flex flex-col md:flex-row gap-md items-center relative overflow-hidden group hover:border-outline transition-colors'>
              ...
              <div className='flex flex-col gap-xs'>
                ...
                <p className='font-body-sm text-body-sm text-on-surface-variant'>
                  Keep track of deployment status, volunteer hours, and community impact metrics. Visualize your entire operational footprint.
                </p>
              </div>
            ...
            <div className='col-span-1 md:col-span-2 bg-surface-container rounded-xl border border-surface-variant p-lg flex flex-col md:flex-row gap-md items-center relative overflow-hidden group hover:border-outline transition-colors'>
              ...
              <div className='flex flex-col gap-xs z-10'>
                ...
                <p className='font-body-sm text-body-sm text-on-surface-variant'>
                  Built to support local community deployments, diverse teams, and organizational standards securely and simply.
                </p>
              </div>
      ...
      {/* Footer */}
      <footer className='w-full border-t border-slate-900 bg-slate-950 font-manrope text-sm font-normal text-blue-500 dark:text-blue-400'>
        <div className='max-w-7xl mx-auto py-12 px-6 flex flex-col md:flex-row justify-between items-center gap-8 text-center md:text-left border-t border-slate-800/60'>
          <div className='flex flex-col gap-2'>
            ...
            <div className='text-slate-500'>
              © 2026 VolunteerMatch. Volunteer Coordination.
            </div>
          </div>
          <div className='flex flex-wrap justify-center md:justify-end gap-6'>
            ...
            <a className='text-slate-500 hover:text-blue-400 transition-colors hover:underline underline-offset-4' href='#'>Compliance</a>
            <a className='text-slate-500 hover:text-blue-400 transition-colors hover:underline underline-offset-4' href='#'>Security Trust Center</a>
          </div>
        </div>
      </footer>
    </div>
